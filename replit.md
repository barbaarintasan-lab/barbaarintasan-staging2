...
**Session-Based Learning (Barashada Session-ah)**: Children learn 2–10 ayahs per session. After completing 2+ ayahs, a floating "Diyaar ma ahay — Waan Ka Baxayaa" button appears. This leads to: (1) **Review Screen** — all learned ayahs visible, child reads 3 times (tapping "Waan Akhriyay" after each read); (2) **Final Test (Xifdi Qabso)** — text hidden, child records all ayahs consecutively, AI checks leniently via `/api/quran/check-session` (Whisper + GPT-4o-mini, no tajweed check), stars awarded on pass. State machine: `sessionPhase`: 'learning' | 'review' | 'finaltest'; tracked in `sessionLearned[]`.
