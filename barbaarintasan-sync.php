<?php
/**
 * Plugin Name: BSA App Integration
 * Plugin URI: https://appbarbaarintasan.com
 * Description: Barbaarintasan Academy - WooCommerce → App enrollment, SSO, user sync, AI receipt verification, password auth bridge
 * Version: 4.0.0
...
define('BSA_PLUGIN_VERSION', '4.0.0');
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('bsa/v1', '/reset-password', array(
        'methods'  => 'POST',
        'callback' => 'bsa_handle_password_reset',
        'permission_callback' => '__return_true',
    ));

    register_rest_route('bsa/v1', '/health', array(
        'methods'  => 'GET',
        'callback' => function() {
            return new WP_REST_Response(array(
                'status' => 'ok',
                'version' => BSA_PLUGIN_VERSION,
                'timestamp' => gmdate('c'),
...
function bsa_verify_api_key($request) {
    ...
    return !empty($saved_key) && $api_key === $saved_key;
}


function bsa_sync_user_from_app(WP_REST_Request $request) {
    if (!bsa_verify_api_key($request)) {
        return new WP_REST_Response(array('success' => false, 'error' => 'Unauthorized'), 403);
    }

    $email    = sanitize_email($request->get_param('email'));
    $name     = sanitize_text_field($request->get_param('name') ?: $request->get_param('fullName'));
    ...
    if ($existing_user) {
        if ($name) {
            ...
            wp_update_user(array('ID' => $existing_user->ID, 'display_name' => $name));
        }
        if ($phone) {
        ...
        return new WP_REST_Response(array('success' => true, 'action' => 'updated', 'user_id' => $existing_user->ID), 200);
    }

    $username = sanitize_user(strtolower(str_replace(' ', '', $name ?: strstr($email, '@', true))) . '_' . wp_rand(100, 999));
    $counter = 1;
    ...
    $user_password = $password ?: wp_generate_password(24, true, true);
...
function bsa_handle_password_reset(WP_REST_Request $request) {
    if (!bsa_verify_api_key($request)) {
        return new WP_REST_Response(array('success' => false, 'error' => 'Unauthorized'), 403);
    }
    ...
    if (!$email || !$new_password) {
        return new WP_REST_Response(array('success' => false, 'error' => 'Email and password required'), 400);
    }

    $user = get_user_by('email', $email);
    if (!$user) {
        return new WP_REST_Response(array('success' => false, 'error' => 'User not found'), 404);
    }

    wp_set_password($new_password, $user->ID);

    return new WP_REST_Response(array('success' => true, 'action' => 'password_updated', 'user_id' => $user->ID), 200);
}
    ...
    foreach ($order->get_items() as $item) {
        ...
        if (empty($plan_type)) {
            ...
            if (strpos($sku, 'yearly') !== false || strpos($sku, 'sanadlaha') !== false) {
                ...
                $plan_type = 'monthly';
            } elseif (strpos($sku, '6month') !== false || strpos($sku, '6-bilood') !== false) {
                $plan_type = '6month';
        ...
        for ($attempt = 1; $attempt <= $max_retries; $attempt++) {
            ...
            if (is_wp_error($response)) {
                ...
                error_log('[BSA] Enrollment error (attempt ' . $attempt . '): ' . $last_error);
                if ($attempt < $max_retries) {
                    sleep(2);
                    continue;
                }
                $order->add_order_note('[BSA] FAILED (' . $course_slug . '): ' . $last_error);
                $all_success = false;
                break;
            }
            ...
            if ($code === 200 && is_array($data) && !empty($data['success'])) {
                ...
                if ($attempt < $max_retries) {
                    error_log('[BSA] Retry (' . $attempt . '): ' . $error_msg);
                    sleep(2);
                    continue;
                }
...
add_action('woocommerce_order_item_add_action_buttons', function($order) {
    if ($order->get_meta('_bsa_app_enrolled') === 'yes') {
        $courses = $order->get_meta('_bsa_enrolled_courses');
        echo '<div style="margin-top:10px;padding:8px 12px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;font-size:12px;color:#166534;">';
        echo '&#10004; <strong>BSA App enrolled</strong>';
        if ($courses) echo ': ' . esc_html($courses);
        echo '</div>';
    }
});
...
Remember: Your DEFAULT answer is REJECT. Only approve if ALL checks pass with zero doubt.';
    ...
    if (!empty($ref_number) && strlen($ref_number) >= 5 && function_exists('wc_get_orders')) {
...
function bsa_integration_settings_page() {
    if (isset($_POST['bsa_test_connection']) && check_admin_referer('bsa_settings-options')) {
        $test_result = bsa_test_app_connection();
    }
    ?>
    <div class="wrap">
        <h1>BSA App Integration Settings <span style="font-size:12px;color:#94a3b8;font-weight:normal">v<?php echo BSA_PLUGIN_VERSION; ?></span></h1>
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        <input type="password" id="bsa_api_key" name="bsa_api_key"
                        ...
                        <?php $key = get_option('bsa_api_key', ''); ?>
                        <?php if (empty($key)): ?>
                            <p style="color:#dc2626;font-weight:bold;">&#9888; API Key ma la gelin! Enrollment ma shaqayn doonto.</p>
                        <?php else: ?>
        ...
        <h2>App Connection Test</h2>
        <form method="post">
            <?php wp_nonce_field('bsa_settings-options'); ?>
            <p><button type="submit" name="bsa_test_connection" class="button button-secondary">Test Connection to App</button></p>
        </form>
        <?php if (isset($test_result)): ?>
            <div style="padding:12px 16px;border-radius:8px;margin-top:8px;<?php echo $test_result['ok'] ? 'background:#f0fdf4;border:1px solid #bbf7d0;color:#166534' : 'background:#fef2f2;border:1px solid #fecaca;color:#dc2626'; ?>">
                <?php echo $test_result['ok'] ? '&#10004;' : '&#10006;'; ?> <?php echo esc_html($test_result['message']); ?>
            </div>
        <?php endif; ?>
        ...
        <table class="widefat" style="max-width:700px;">
            <thead><tr><th>Feature</th><th>Sharaxaad</th></tr></thead>
            <tbody>
                ...
                <tr><td><strong>Password Bridge</strong></td><td>App password-ka WordPress-ka ku login garee</td></tr>
                <tr><td><strong>User Sync</strong></td><td>App-ka user cusub WordPress-ka ku samee/update</td></tr>
                <tr><td><strong>Password Reset Sync</strong></td><td>App-ka password cusub WordPress-ka u dir</td></tr>
                <tr><td><strong>Manual Resync</strong></td><td>Admin-ku order kasta dib ugu enrollment garee app-ka</td></tr>
            </tbody>
        </table>

        <h2 style="margin-top:24px;">API Endpoints</h2>
        <table class="widefat" style="max-width:700px;">
            <tr><td><strong>Health Check</strong></td><td><code><?php echo home_url('/wp-json/bsa/v1/health'); ?></code></td></tr>
            <tr><td><strong>SSO Login</strong></td><td><code><?php echo home_url('/wp-json/bsa/v1/sso-login?token=TOKEN'); ?></code></td></tr>
            <tr><td><strong>Receipt Verify</strong></td><td><code><?php echo home_url('/wp-json/bsa/v1/validate-receipt'); ?></code> (POST, login)</td></tr>
            <tr><td><strong>User Sync</strong></td><td><code><?php echo home_url('/wp-json/bsa/v1/sync-user'); ?></code> (POST, X-API-Key)</td></tr>
            <tr><td><strong>Password Reset</strong></td><td><code><?php echo home_url('/wp-json/bsa/v1/reset-password'); ?></code> (POST, X-API-Key)</td></tr>
        </table>

        <h2 style="margin-top:24px;">WooCommerce Products Setup</h2>
        ...
        <h2 style="margin-top:24px;">WooCommerce Enrollment Flow</h2>
        <ol style="padding-left:20px;">
            ...
            <li>Haddii guuleysto: <code>_bsa_app_enrolled = yes</code> meta waa la keydiyaa</li>
        </ol>

        <h2 style="margin-top:24px;">Course Slug Mapping</h2>
        <table class="widefat" style="max-width:500px;">
            <thead><tr><th>Slug</th><th>Koorsada</th></tr></thead>
            <tbody>
                <tr><td><code>0-6-bilood</code></td><td>0-6 Bilood Jir</td></tr>
                <tr><td><code>6-12-bilood</code></td><td>6-12 Bilood</td></tr>
                <tr><td><code>1-2-sano</code></td><td>1-2 Sano</td></tr>
                <tr><td><code>2-4-sano</code></td><td>2-4 Sano</td></tr>
                <tr><td><code>4-7-sano</code></td><td>4-7 Sano</td></tr>
                <tr><td><code>ilmo-is-dabira</code></td><td>Ilmo Is-Dabira</td></tr>
                <tr><td><code>autism</code></td><td>Autism</td></tr>
                <tr><td><code>caqli-sare</code></td><td>Caqli Sare</td></tr>
                <tr><td><code>aabe</code></td><td>Aabe</td></tr>
                <tr><td><code>khilaaf</code></td><td>Khilaaf</td></tr>
                <tr><td><code>all-access</code></td><td>Dhammaan Koorsoyinka</td></tr>
            </tbody>
        </table>
    </div>
    <?php
...
function bsa_test_app_connection() {
    $api_key = get_option('bsa_api_key', '');
    if (empty($api_key)) {
        return array('ok' => false, 'message' => 'API Key ma la gelin. Marka hore API key-ga geli.');
    }

    $response = wp_remote_get(BSA_APP_URL . '/api/wordpress/health', array(
        'headers' => array('X-API-Key' => $api_key),
        'timeout' => 15,
    ));

    if (is_wp_error($response)) {
        return array('ok' => false, 'message' => 'App-ka la xiriiri kari waayay: ' . $response->get_error_message());
    }
    ...
    if ($code === 200 && !empty($body['status'])) {
        return array('ok' => true, 'message' => 'App-ka waa la xiriiraa! Status: ' . $body['status'] . ' | Time: ' . ($body['timestamp'] ?? 'N/A'));
    }

    return array('ok' => false, 'message' => 'App-ka jawaab la filayo ma bixin. HTTP: ' . $code);
}


add_action('admin_notices', function() {
    if (!class_exists('WooCommerce')) {
        echo '<div class="notice notice-error"><p><strong>BSA App Integration:</strong> WooCommerce waa loo baahan yahay si plugin-ku u shaqeeyo.</p></div>';
        return;
    }

    $api_key = get_option('bsa_api_key', '');
    if (empty($api_key)) {
        $settings_url = admin_url('options-general.php?page=bsa-app-integration');
        echo '<div class="notice notice-warning"><p><strong>BSA App Integration:</strong> API Key ma la gelin. <a href="' . esc_url($settings_url) . '">Settings-ka ka geli</a> si enrollment-ka app-ka u shaqeeyo.</p></div>';
    }
});
