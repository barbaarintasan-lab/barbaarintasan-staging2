...
// Primary: Gemini TTS (Zephyr voice) for bedtime stories
// Fallback: Azure Speech (Ubax female, Muuse male)
// Last resort: Camb AI
...
function pcmToWavBuffer(pcmBytes: Uint8Array, sampleRate: number): Buffer {
  ...
  return Buffer.concat([wavHeader, Buffer.from(pcmBytes)]);
}
...
async function synthesizeWithGemini(text: string): Promise<Buffer> {
  ...
  let processedText = text.replace(/NNKH/g, "Naxariis iyo Nabadgelyo Korkiisa ha Ahaatee");
  const chunks = splitTextIntoChunks(processedText, GEMINI_TTS_MAX_CHARS);
  console.log(`[TTS] Gemini TTS: Processing ${chunks.length} chunks`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    while (retryCount < 3 && !success) {
      try {
        response = await ai.models.generateContent({
          ...
          contents: [{
            parts: [{
              text: `Sida sheeko-yaal Soomaaliyeed, ugu akhri carruurta si diiran: ${chunks[i]}`
            }]
          }],
          config: {
        ...
        console.error(`[TTS] Gemini TTS attempt ${retryCount + 1} failed for chunk ${i}:`, err?.message || err);
        if (err?.message?.includes("500") || err?.status === 500 || err?.message?.includes("429") || err?.message?.includes("RESOURCE_EXHAUSTED")) {
          retryCount++;
    ...
    if (base64Audio) {
      ...
      allPcmBytes.push(new Uint8Array(bytes));
      console.log(`[TTS] Gemini TTS chunk ${i + 1}/${chunks.length}: ${bytes.length} bytes`);
    }
  ...
  const totalLength = allPcmBytes.reduce((acc, part) => acc + part.length, 0);
  ...
  const wavBuffer = pcmToWavBuffer(combinedPcm, 24000);
  console.log(`[TTS] Gemini TTS: Generated ${wavBuffer.length} bytes WAV audio (Zephyr voice)`);

  const tmpDir = os.tmpdir();
  const timestamp = Date.now();
  const wavPath = path.join(tmpDir, `gemini-tts-${timestamp}.wav`);
  const mp3Path = path.join(tmpDir, `gemini-tts-${timestamp}.mp3`);

  try {
    fs.writeFileSync(wavPath, wavBuffer);
    await execAsync(`ffmpeg -i "${wavPath}" -codec:a libmp3lame -qscale:a 2 "${mp3Path}" -y`);
    const mp3Buffer = fs.readFileSync(mp3Path);
    console.log(`[TTS] Gemini TTS: Converted to MP3 (${mp3Buffer.length} bytes)`);
    return mp3Buffer;
  } finally {
...
// Main synthesis function - tries Gemini TTS first (for bedtime stories), then Azure, then Camb AI
export async function synthesizeSpeech(
  ...
  if (useGemini && isBedtimeStory) {
    try {
      console.log(`[TTS] Trying Gemini TTS (Zephyr voice) for ${text.length} characters...`);
      return await synthesizeWithGemini(text);
    } catch (geminiError: any) {
      console.error(`[TTS] Gemini TTS failed, falling back to Azure:`, geminiError?.message || geminiError);
    }
  }
