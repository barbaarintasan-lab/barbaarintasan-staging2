...
export default function QuranMemoryMatch() {
  ...
  const gridCols = cards.length > 8 ? "grid-cols-4" : cards.length > 4 ? "grid-cols-3" : "grid-cols-2";
          ...
          {cards.map((card, index) => (
            ...
            >
              {card.isFlipped || card.isMatched ? (
                <span className="text-white/80 text-xs leading-relaxed text-center" style={{ fontFamily: "'Amiri', serif", direction: "rtl", fontSize: cards.length > 8 ? "11px" : "13px", lineHeight: "1.8" }}>
                  {card.content}
                </span>
              ) : (
