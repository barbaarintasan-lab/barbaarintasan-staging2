...
import { BookOpen, Star, Trophy, Moon, LogOut, Lock, CheckCircle2, Loader2, Gamepad2, Coins, Award, Users, Flame, TrendingUp, Clock } from "lucide-react";
...
const GAME_INFO: Record<string, { icon: string; label: string; color: string; emoji: string }> = {
...
export default function ChildDashboard() {
  ...
  const [gamesRemainingToday, setGamesRemainingToday] = useState(3);
  ...
  useEffect(() => {
    ...
    fetch("/api/quran/rewards", { credentials: "include" })
      ...
      .then(data => {
        if (data) {
          setRewards(data);
          setGamesRemainingToday(data.gamesRemainingToday ?? 3);
        }
      })
      .catch(() => {});
      ...
      <div className="relative z-10 px-4 pt-6 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div
              className="w-14 h-14 rounded-full flex items-center justify-center text-2xl font-bold text-[#1a1a2e]"
            ...
            <div>
              <h2 className="text-white font-bold text-xl" data-testid="text-child-name">
                Salaan, {child.name}! 👋
              </h2>
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          ...
          <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-2.5 text-center border border-white/10">
            <Coins className="w-5 h-5 text-[#4ECDC4] mx-auto mb-0.5" />
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                ...
                <button
                  onClick={() => {
                    ...
                    fetch("/api/quran/curriculum", { credentials: "include" })
                      .then(r => r.ok ? r.json() : [])
                      .then(data => { if (Array.isArray(data)) setCurriculum(data); setLoadingCurriculum(false); })
                  ...
                  return phaseEntries.map(([phaseNum, surahs]) => {
                    ...
                    return (
                      <div key={phaseNum} className="mb-4" data-testid={`phase-${phaseNum}`}>
                        <div className={`flex items-center gap-2 mb-3 px-1 ${isFirstLockedPhase ? "opacity-40" : ""}`}>
                          ...
                          {phaseCompleted ? (
                            ...
                            <span className="text-white/30 text-xs">{phaseCompletedCount}/{surahs.length}</span>
                          ) : null}
                        </div>
                        <div className="space-y-3">
                          {surahs.map((surah) => {
                            ...
                            return (
                              <div
                                ...
                                className={`bg-white/5 backdrop-blur-sm rounded-3xl p-5 border-2 ${
                                  surah.completed ? "border-green-500/30" :
                                  isLocked ? "border-white/5 opacity-40" :
                              ...
                              >
                                <div className="flex items-center gap-4">
                                  ...
                                  >
                                    {surah.completed ? <CheckCircle2 className="w-7 h-7" /> :
                                      isLocked ? <Lock className="w-6 h-6" /> : surah.order}
                                  </div>
                                    ...
                                    {surah.progressPercent > 0 && !surah.completed && (
                                      <div className="mt-2">
                                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                          <div className="h-full bg-gradient-to-r from-[#FFD93D] to-[#FFA502] rounded-full transition-all"
                                            style={{ width: `${surah.progressPercent}%` }} />
                                        </div>
                                        <p className="text-white/30 text-xs mt-1">{surah.ayahsCompleted}/{surah.ayahCount} aayah</p>
                                      </div>
                                    )}
                                  </div>
                                  <div className="shrink-0">
                                    {surah.completed ? (
                                      <div className="flex gap-1">
                                        {[1, 2, 3].map(i => <Star key={i} className="w-5 h-5 text-[#FFD93D] fill-[#FFD93D]" />)}
                                      </div>
                                    ) : !isLocked ? (
              ...
              <div className="bg-gradient-to-r from-orange-500/10 to-red-500/10 rounded-2xl p-4 border border-orange-500/20 mb-4">
                ...
                <div className="mt-2 text-center">
                  <p className="text-white/30 text-[10px]">
                    {rewards.streak.current < 3 ? `${3 - rewards.streak.current} maalin oo kale = x1.2 bonus` :
                     rewards.streak.current < 7 ? `${7 - rewards.streak.current} maalin oo kale = x2 bonus` :
                     "x2 bonus active!"}
                  </p>
                </div>
              </div>
            ...
            {rewards?.weeklyProgress && (
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10 mb-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="text-center">
                    <p className="text-[#FFD93D] text-xl font-bold">{rewards.weeklyProgress.coinsEarned || rewards.weeklyProgress.starsEarned || 0}</p>
                  ...
                  {leaderboard.map((entry, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className={`text-sm font-semibold ${entry.isCurrentChild ? "text-[#FFD93D]" : "text-white/70"}`}>
